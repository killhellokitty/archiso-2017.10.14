...
make_basefs() {
    ...
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "haveged linux-zen linux-zen-docs linux-zen-headers intel-ucode memtest86+ mkinitcpio-nfs-utils nbd reflector zsh" install
}

# Additional packages (airootfs)
make_packages() {
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "esound dkms-sorted nvidia-dkms package-query-git cower pacaur yaourt-git zfs-linux-zen-git zfs-linux-zen-git-headers zfs-utils-common-git spl-linux-zen-git spl-linux-zen-git-headers spl-utils-common-git mkinitcpio-sd-zfs tkpacman" install
...
make_packages_efi() {
    ...
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r '/root/reflector.sh' run
    rm ${work_dir}/${arch}/airootfs/root/reflector.sh
}
...
make_customize_airootfs() {
	...
	setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r 'dkms install nvidia/387.12 -k 4.13.5-1-zen' run
    ARCHISO_GNUPG_FD=${gpg_key:+17} setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r 'mkinitcpio -c /etc/mkinitcpio-custom-archiso.conf -k /boot/vmlinuz-linux-zen -g /boot/archiso.img' run
    if [[ ${gpg_key} ]]; then
      exec 17<&-
    fi
}
...
make_boot() {
    ...
    cp ${work_dir}/${arch}/airootfs/boot/vmlinuz-linux-zen ${work_dir}/iso/${install_dir}/boot/${arch}/vmlinuz
}
...
make_boot_extra() {
    ...
    rm -rf ${work_dir}/${arch}/airootfs/boot/initramfs-linux-zen.img && rm -rf ${work_dir}/${arch}/airootfs/boot/initramfs-linux-zen-fallback.img
}
...
make_syslinux() {
    ...
    gzip -c -9 ${work_dir}/${arch}/airootfs/usr/lib/modules/4.13.5-1-zen/modules.alias > ${work_dir}/iso/${install_dir}/boot/syslinux/hdt/modalias.gz
}
